package com.dev43.cartax.graph.provider;
...
import com.dev43.cartax.CostData;
import com.dev43.cartax.graph.ColumnSchema;
...
public class DataContentProvider extends ContentProvider {
	...
	static final String AUTHORITY = "com.dev43.cartax.provider";
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		if (ColumnSchema.Aspect.DATASET_ASPECT_AXES.equals( uri.getQueryParameter(ColumnSchema.DATASET_ASPECT_PARAMETER) )) {
			...
			int row_index = 0;
			for (int i=0; i<CostData.DEMO_AXES_LABELS.length; i++) {

				c.newRow().add( row_index ).add( CostData.DEMO_AXES_LABELS[i] );
				row_index++;
			}
		...
		return c;
			...
			// Add x-axis data
			for (int i=0; i<CostData.DEMO_X_AXIS_DATA.length; i++) {
				...
				.add( CostData.DEMO_X_AXIS_DATA[i] )
				.add( null );
